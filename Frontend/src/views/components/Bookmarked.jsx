...
export default function Bookmarked({ userData, scrollRef }) {
    ...
    const getPosts = async () => {
        ...
        try {
            setIsLoading(true);
            const lastFeedId = userPost.length == 0 ? null : userPost[userPost.length - 1]?.post_data?.id;
            const url = (lastFeedId) ?
                `${backend_url}/api/post/profile?userId=${userData.id}&&limit=${10}&&lastFeedId=${lastFeedId}&&reqType=${'saved_posts'}`
                :
                `${backend_url}/api/post/profile?userId=${userData.id}&&limit=${10}&&reqType=${'saved_posts'}`
        ...
        <>
            <Stack width={'100%'} spacing={2} position={'relative'} mt={2}>
                ...
                {userPost.map((p) =>
                    ...
                    <PostUI key={p?.post_data?.id} followed={p.user_data?.followingStatus} postData={p?.post_data} userData={p.user_data} />
