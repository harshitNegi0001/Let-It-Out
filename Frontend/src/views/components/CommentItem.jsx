...
function CommentItem({ commentData, level }) {
        ...
        <>
            <Box width={'100%'} p={1} sx={{ display: 'flex', gap: 1 }} position={'relative'} border={1} borderColor={'divider'} borderRadius={2}>
                ...
                <Box width={{ xs: 'calc(100% - 50px)', sm: 'calc(100% - 60px)' }} sx={{ display: 'flex', flexDirection: 'column', gap: '5px' }}>
                    ...
                    <Box width={'100%'} >
                        {commentData?.replying_to && <Typography sx={{ display: 'flex', gap: '3px', alignItems: 'center' }} fontSize={{ xs: '7px', sm: '12px' }} color="text.secondary" fontWeight={'200'}>
                            · replying to <Typography onClick={() => navigate(`/profile/${commentData?.replying_to}`)} sx={{ '&:hover': { textDecoration: 'underline' }, cursor: 'pointer', }} fontSize={{ xs: '7px', sm: '12px' }} color="text.secondary" fontWeight={'200'} >@{commentData?.replying_to}</Typography>
                        </Typography>}
                    ...
                    <Stack direction={'row'} justifyContent={'start'} spacing={3} px={{ xs: 1, sm: 2 }}  >
                        ...
                        <Box display={'flex'} width={'120px'} justifyContent={'start'} alignItems={'center'}>
                            <IconButton size="small" title="comments">
                                <CommentsIcon sx={{ fontSize: { xs: '14px', sm: '20px' } }} />

                            </IconButton><Typography variant="body2" fontSize={12} component={'span'}>{commentData?.comments_count||0}</Typography>
                        </Box>
                    ...
                    {(commentData?.comments_count > 0) && level == 1 && <>
                        ...
                        {openComment &&
                            commentData?.childComments?.map(c =>
                                <CommentItem key={c.id} commentData={c} level={2} />
                            )
            ...
            {(commentData?.comments_count > 0) && level > 1 && <>
                ...
                {openComment &&
                    commentData?.childComments?.map(c =>
                        <CommentItem key={c.id} commentData={c} level={3} />
                    )
